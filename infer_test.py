...
def build_net(cfg: Config, device: str) -> torch.nn.Module:
    ...
    return build_training_model(cfg, pretrained=False).to(device)
...
def infer_one(name: str, cfg: Config, df: pd.DataFrame, splits: list, device: str):
    ...
    net.load_state_dict(load_model_weights(name, results_dir=cfg.results_dir))
    net.eval()
...
def main():
    ...
    splits = load_splits(cfg0.splits_path, cfg0.labels_path, len(df))
    device = get_device()

    sel = [matrix[args.task_id]] if args.task_id is not None else matrix
    for name, cfg in sel:
        if not completed(name, cfg.results_dir):
